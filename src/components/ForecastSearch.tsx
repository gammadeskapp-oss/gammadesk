...
export function ForecastSearch({ initial = '' }: { initial?: string }) {
  ...
  const go = (symbol: string) => {
    ...
    if (!clean) return;
    startTransition(() => router.push(`/forecast?symbol=${encodeURIComponent(clean)}`));
  };
  ...
  return (
    <div className="space-y-2.5">
      ...
      >
        <label htmlFor="forecast-ticker" className="sr-only">
          Ticker symbol
        </label>
        <input
          id="forecast-ticker"
          ...
          placeholder="SIMULATE ANY TICKER — SPY, NVDA, AAPL…"
          autoComplete="off"
          spellCheck={false}
          maxLength={10}
