...
const scanStore = createJsonStore<StoredScans>(
  /*
   * A separate document from the live scanner's.
   *
   * Not a tuning choice — a correctness one. The two scanners store different
   * shapes under the same key, and each one's validator rejects the other's
   * documents, so sharing the path would have them overwriting each other all
   * day and both reporting "no scan today". The gamma document is shared,
   * because that shape has not changed and this page is meant to read what the
   * live 08:30 job already fetched rather than spend chains of its own.
   */
  'gammadesk/previousscanner-scans.json',
  () => ({ scans: [] }),
  (raw) =>
