...
export function DataQuality({ meta, contracts }: DataQualityProps) {
  ...
  const totalIv = ivSources.api + ivSources.solved + ivSources.model;
  ...
  return (
    <div className="panel px-3 py-2 text-2xs">
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5">
        <Stat
          label="source"
          value={meta.source === 'polygon' ? 'Polygon.io (end of day)' : 'generated sample'}
        />
        <Stat label="quote date" value={meta.quoteDateLabel} />
        <Stat label="contracts" value={String(contracts)} />
        <Stat
          label="IV"
          value={`${ivSources.api} api / ${ivSources.solved} solved / ${ivSources.model} modelled`}
        />
        <Stat label="api calls" value={String(meta.polygonRequests)} />
      ...
      {modelledPct >= 25 && (
        <p className="mt-2 border-t border-term-line pt-2 text-flip/80">
          {modelledPct}% of contracts had no usable implied volatility from the API
          and fall back to a modelled volatility surface. Treat the exposure
